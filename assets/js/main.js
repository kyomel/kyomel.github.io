...
(function () {
  ...
  function printBanner() {
    ...
    const lines =
      window.innerWidth < 480 ? BANNER_MOBILE : BANNER;
  ...
  async function printWelcome() {
    printBanner();
    printBlank();
    await typeLine(`Welcome to ${HOST}'s portfolio shell.`, "line", isCoarse ? 8 : 10);
  ...
  async function runBoot() {
    ...
    try {
      for (const line of BOOT_LINES) {
        ...
        await sleep(isCoarse ? Math.max(20, line.delay * 0.7) : line.delay, signal);
      }
